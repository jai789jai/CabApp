...
namespace CabApp.Core.Implementation
{
    public class DataService: IDataService
    {
        ...
        public async Task<bool> ChangeCabLocationAsync(int cabId, int newLocationId)
        {
            try
            {
                var cab = await GetCabByIdAsync(cabId);
                if (cab == null)
                    return false;

                // Verify the new location exists
                var location = await GetLocationByIdAsync(newLocationId);
                if (location == null)
                    return false;
        ...
        public async Task<List<CabDetails>> GetAvailableCabsAtLocationAsync(int locationId)
        {
            try
            {
                var allCabs = await GetAllCabsAsync();
        ...
        public async Task<CabDetails?> BookCabForTripAsync(int tripId, int fromLocationId)
        {
            try
            {
                ...
                var cabUpdated = await UpdateCabAsync(selectedCab);
                var tripUpdated = await UpdateTripAsync(trip);
        ...
        public async Task<bool> CompleteTripAsync(int tripId)
        {
            try
            {
                ...
                var cab = await GetCabByIdAsync(trip.AssignedCabId.Value);
                if (cab == null)
                    return false;
                ...
                // Update both records
                var tripUpdated = await UpdateTripAsync(trip);
                var cabUpdated = await UpdateCabAsync(cab);
